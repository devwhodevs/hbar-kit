...
const transport = (handler: (path: string) => unknown): Transport => ({
...
describe("transactions.find", () => {
  it("builds the query and returns a normalized page", async () => {
    ...
    const t = transport((path) => {
      captured = path
      return parseJsonWithBigInt(raw("tx-list.json"))
    })
  ...
  it("serializes after/before as timestamp gte/lt filters", async () => {
    ...
    await createTransactionsResource(t).find({
      accountId: "0.0.1",
      after: new Date(1748625594029),
      before: new Date(1748625599000),
    })
...
describe("transactions.get", () => {
  it("converts @-form to dash, returns the array", async () => {
    ...
    const t = transport((path) => {
      captured = path
      return parseJsonWithBigInt(raw("single-tx.json"))
    })
