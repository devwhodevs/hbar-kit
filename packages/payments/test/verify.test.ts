...
function clientReturning(listBody: unknown, tokenBody?: unknown) {
  const fetchMock = async (url: string) => {
    if (url.includes("/api/v1/tokens/")) {
      return new Response(JSON.stringify(tokenBody ?? { token_id: "0.0.5449", decimals: "6", symbol: "USDC", name: "USD Coin", type: "FUNGIBLE_COMMON" }), { status: 200 })
    }
...
const htsListRaw = JSON.parse(readFileSync(fileURLToPath(new URL("../../mirror/test/fixtures/tx-list.json", import.meta.url)), "utf8"))

describe("verifyHtsPayment", () => {
  it("confirms an exact HTS payment with matching memo", async () => {
    const r = await verifyHtsPayment({
      client: clientReturning(htsListRaw), receiver: "0.0.8093816", tokenId: "0.0.5449", decimals: 6,
      amount: "2.889029", memo: "order_6471727153206", network: "testnet",
    })
  ...
  it("auto-fetches decimals when omitted", async () => {
    const r = await verifyHtsPayment({
      client: clientReturning(htsListRaw), receiver: "0.0.8093816", tokenId: "0.0.5449",
      amount: "2.889029", memo: "order_6471727153206", network: "testnet",
    })
    expect(r.matched).toBe(true)
  })
  ...
  it("returns mismatch on wrong memo", async () => {
    const r = await verifyHtsPayment({
      client: clientReturning(htsListRaw), receiver: "0.0.8093816", tokenId: "0.0.5449", decimals: 6,
      amount: "2.889029", memo: "wrong_memo", network: "testnet",
    })
  ...
  it("returns underpaid when amount is short", async () => {
    const r = await verifyHtsPayment({
      client: clientReturning(htsListRaw), receiver: "0.0.8093816", tokenId: "0.0.5449", decimals: 6,
      amount: "5.0", memo: "order_6471727153206", network: "testnet",
    })
    expect(r.matched).toBe(false)
  ...
  it("returns pending when nothing matches the receiver", async () => {
    const r = await verifyHbarPayment({ client: clientReturning({ transactions: [], links: { next: null } }), receiver: "0.0.404", amount: "1", network: "testnet" })
    expect(r.matched).toBe(false)
    expect(r.status).toBe("pending")
  })

  it("flags duplicates when two transactions each satisfy the request", async () => {
    const dup = { transactions: [htsListRaw.transactions[0], { ...htsListRaw.transactions[0], transaction_id: "0.0.6628041-1780150228-000000000", consensus_timestamp: "1780150232.000000000" }], links: { next: null } }
    const r = await verifyHtsPayment({
      client: clientReturning(dup), receiver: "0.0.8093816", tokenId: "0.0.5449", decimals: 6,
      amount: "2.889029", memo: "order_6471727153206", network: "testnet",
    })
